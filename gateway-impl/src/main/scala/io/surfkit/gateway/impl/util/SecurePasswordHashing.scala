package io.surfkit.gateway.impl.util
...
object SecurePasswordHashing {
  ...
  def validatePassword(password: String, hashedPassword: String): Boolean = {
    ...
    val hash = Try(Base64.getDecoder.decode(hashParts(1))).toEither
    val salt = Try(Base64.getDecoder.decode(hashParts(2))).toEither
